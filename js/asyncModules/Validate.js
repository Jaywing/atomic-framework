  ...
  hasError(f) {
    ...
    for (let key in msg) {
      if (v[key]) {
        ...
        if (key === "tooShort")
          return msg[key].replace("*value*", f.getAttribute("minLength"));
        if (key === "tooLong")
          return msg[key].replace("*value*", f.getAttribute("maxLength"));
        if (key === "rangeOverflow")
          return msg[key].replace("*value*", f.getAttribute("max"));
        if (key === "rangeUnderflow")
          return msg[key].replace("*value*", f.getAttribute("min"));

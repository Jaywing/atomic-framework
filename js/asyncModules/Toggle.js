  ...
  init() {
    ...
    if (this.settings.target === "") {
      ...
      this.el.setAttribute(
        "aria-pressed",
        this.el.classList.contains(this.settings.activeClass)
      ); // init aria
    } else {
      this.self = false;
    ...
    const toggleIt = () => {
      ...
      if (this.self) {
        ...
        this.el.setAttribute(
          "aria-pressed",
          Toggle.el.classList.contains(this.settings.activeClass)
        ); // init aria
        return;
      }
    ...
    if (target) {
      if (this.settings.hide) {
        if (target.hasAttribute("hidden")) {
          ...
          window.jwAtomic.modules[
            baseEle.getAttribute("id")
          ].details.active = true;
        } else {
      ...
      window.jwAtomic.modules[
        baseEle.getAttribute("id")
      ].details.active = target.classList.contains(this.settings.activeClass);
    }
  }
}
